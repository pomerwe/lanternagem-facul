...
namespace lanternagem_api.Interfaces
{
  ...
  {
    Task<(bool IsSuccess, List<WorkOrder> WorkOrders, string ErrorMessage)> GetWorkOrdersByCustomerCNPJ(string CNPJ);
    ...
    Task<(bool IsSuccess, string ErrorMessage)> AddWorkOrder(WorkOrder workOrder);
    Task<(bool IsSuccess, string ErrorMessage)> UpdateWorkOrder(WorkOrder workOrder);
